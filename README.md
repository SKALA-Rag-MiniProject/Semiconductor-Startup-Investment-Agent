...
Agentic RAG에서 가장 중요한 것은 모델 성능 뿐만 아니라 **상태 설계(state)**, **출력 형식 강제(JSON)**, **fallback 전략**이었습니다. 멀티 에이전트 구조에서는 중간 결과를 계속 보고서로 만들기보다, 기업별 분석 결과를 state에 축적하고 **마지막에 한 번만 보고서를 생성**하는 구조가 훨씬 안정적이었습니다. 웹 검색·LLM 응답은 노이즈와 파싱 실패가 빈번하므로, 프롬프트를 압축하고 **재시도 경로**를 두는 것이 실전 품질에 결정적이었습니다. 보완으로 덧붙이면, 사용할 문서와 임베딩할 데이터 자체에 대한 **선정과 전처리**의 중요성도 크게 느꼈습니다.
