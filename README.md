...
13. [Final Key Points](#13-final-key-points)
...
>
> 이 프로젝트는 PDF 기술 문서와 웹 검색 결과를 활용해 AI 반도체 스타트업의 **기술력, 시장성, 경쟁 구도, 리스크**를 단계별로 분석하고, 최종적으로 **투자 유망(INVEST) / 보류(HOLD)** 수준의 종합 의견을 pdf 보고서 형태로 자동 생성합니다. 단순 요약이 아니라 RAG로 회수한 근거를 검증한 뒤, 각 에이전트가 점수와 요약을 쌓고, Scorecard 방식으로 투자 판단을 내리는 구조입니다.
...
> **설계 전환 배경**       
> 
> 처음에는 벤처기업 CSV를 임베딩 벡터로 구성해 두고 검색하는 방식으로 진행했습니다. 그런데 같은 회사인데 여러 건으로 나오거나, 비슷한 기업인데 업종이 상세한 부분에서 달라 거리값이 너무 멀어지는 경우 등 문제가 발견되었습니다. BM25 키워드 기반 검색을 하이브리드로 구축했지만 여전히 같은 문제가 발생해, 이렇게 찾는 것이 맞는지 의문이 들었습니다.
>
> 그래서 유망한 기업을 미리 픽해 두고, 해당 기업들의 논문을 미리 임베딩해 둔 뒤, **그 기업 3개를 비교·분석해 달라**고 요청하는 에이전트로 방향을 잡았습니다.
...
현재 구현은 `state.py`와 `main.py`를 중심으로 한 **상태 기반 멀티 에이전트 파이프라인**으로 동작합니다. 사용자 질문과 대상 기업 목록이 주어지면, 기업별로 RAG 단계(검색 → 검증)를 거친 뒤 기술, 시장, 경쟁, 리스크, 투자판단, 보고서 작성 순으로 에이전트가 실행되며, 모든 중간 결과는 `AgentState`에 누적됩니다.
...
flowchart TD
    ...
    subgraph LOOP[Company Analysis Loop]
        direction LR
...
벤치마크에 앞서 **이미지, 표, 레퍼런스, 페이지 번호** 등 임베딩이 필요하지 않은 요소를 제거하는 데이터 전처리를 수행하였습니다. 이렇게 정제된 텍스트만을 대상으로 임베딩 품질을 평가하였습니다.
...
`agents/utils.py`의 `grade_retrieved_docs()`는 검색 결과가 질의 및 기업과 얼마나 맞는지 세 가지를 합산해 신뢰도를 판단합니다. 기업명이 실제 문서에 얼마나 등장하는지, 평균 검색 점수는 어떤지, 질문 토큰과 문서 내용의 겹침 정도는 어떤지입니다. 이 점수가 설정된 임계값보다 낮으면 재검색을 시도합니다. 그래서 이 프로젝트는 "검색했다"에서 끝나지 않고 **검색 결과를 한 번 더 의심하는 구조**를 갖습니다.
...
Agentic RAG에서 가장 중요한 것은 모델 성능 뿐만 아니라 **상태 설계(state)**, **출력 형식 강제(JSON)**, **fallback 전략**이었습니다. 멀티 에이전트 구조에서는 중간 결과를 계속 보고서로 만들기보다, 기업별 분석 결과를 state에 축적하고 **마지막에 한 번만 보고서를 생성**하는 구조가 훨씬 안정적이었습니다. 웹 검색·LLM 응답은 노이즈와 파싱 실패가 빈번하므로, 프롬프트를 압축하고 **재시도 경로**를 두는 것이 실전 품질에 결정적이었습니다. 

또한, 단순히 웹 기반으로 문서를 긁어와 바로 임베딩을 수행하는 방식은 생각보다 정확도가 높지 않다는 점을 확인했습니다. 실제 실험 과정에서는 **PDF를 얼마나 정제하느냐(이미지·표·머리말·페이지 번호·불필요 레퍼런스 제거 등)에 따라 임베딩 분석의 질이 크게 달라졌고**, 결국 좋은 성능을 내려면 RAG 상단에 있는 **논문 서치 엔진과 PDF 자동 전처리 에이전트**가 함께 설계되어야 검색–임베딩–분석까지 End-to-End 자동화가 가능하다는 인사이트를 얻었습니다. 이 경험을 통해 사용할 문서와 임베딩할 데이터 자체에 대한 **선정과 전처리**의 중요성을 더욱 크게 인식하게 되었습니다.

## 13. Final Key Points

이번 보고서는 기업별로 **기술력, 시장성, 경쟁우위, 리스크**를 동일 기준으로 비교해 투자 우선순위를 도출한 것이 핵심입니다. 특히 기술 문서 기반 RAG 분석 결과를 중심으로 점수화하고, 단일 지표가 아니라 **스코어카드 합산**으로 INVEST/HOLD를 판단해 의사결정 근거를 구조화했습니다. 최종적으로는 "왜 이 기업이 상대적으로 더 투자 매력적인지"와 "어떤 불확실성이 남아 있는지"를 함께 제시해, 추천과 보수적 해석을 동시에 제공했습니다.

- 평가 중간중간 **웹 자료를 증거로 활용**해 평가의 신뢰성을 높였습니다.
- **무한 루프**나 중간 노드 간 전달 과정의 오류를 막기 위한 설계를 우선적으로 반영했습니다.
